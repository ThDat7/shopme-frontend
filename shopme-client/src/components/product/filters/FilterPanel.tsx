...
import { popularFeatures } from '../../../mock/productData';
import { BrandResponse } from '../../../types/brandTypes';
import { CategoryResponse } from '../../../types/categoryTypes';
import { ProductFilterType, PRODUCT_TYPE_LABELS } from '../../../types/productTypes';
...
const FilterPanel: React.FC<FilterPanelProps> = ({
  ...
  const updateTreeData = (list: TreeNode[], id: string, children: TreeNode[]): TreeNode[] => {
  ...
  const features = popularFeatures.map(feature => feature.name);
  ...
  const handleCategorySelect = (categoryId: number, checked: boolean, parentId?: number) => {
          ...
          <div className="flex items-center w-full">
            ...
            <div className="flex items-center">
              ...
              {cat.productCount !== undefined && (
                <span className="ml-1 text-gray-400 text-xs">({cat.productCount})</span>
              )}
            </div>
          </div>
        ),
  ...
  const onLoadData = async (node: any) => {
    ...
    try {
      ...
      const childCategories = await categoryService.getChildCategoryById(parseInt(key));
      ...
      const childNodes = childCategories.map(cat => ({
        title: (
          <div className="flex items-center w-full">
            <Checkbox
              checked={selectedCategoryIds.includes(cat.id)}
              onChange={(e) => handleCategorySelect(cat.id, e.target.checked, parseInt(key))}
              className="mr-2"
            />
            <div className="flex items-center">
              {cat.image && (
          ...
          try {
            ...
            if (category?.parentId) {
              ...
              if (parentId && !loadedKeys.includes(parentId.toString())) {
                try {
                  const childCategories = await categoryService.getChildCategoryById(parentId);
                  ...
                  const childNodes = childCategories.map(cat => ({
                    title: (
                      <div className="flex items-center w-full">
                        <Checkbox
                          checked={selectedCategoryIds.includes(cat.id)}
                          onChange={(e) => handleCategorySelect(cat.id, e.target.checked, parentId)}
                          className="mr-2"
                        />
                        <div className="flex items-center">
                          {cat.image && <Avatar src={cat.image} size={24} shape="square" className="mr-2" />}
                          <span className="text-gray-700">{cat.name}</span>
                          {cat.productCount !== undefined && (
                            <span className="ml-1 text-gray-400 text-xs">({cat.productCount})</span>
                          )}
                  ...
                  setCategoryTree(prev => updateTreeData(prev, parentId.toString(), childNodes));
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
        <Title level={5} className="!mb-3 text-primary-600 font-medium">Loại sản phẩm</Title>
        <Radio.Group 
          value={productType} 
      ...
      {/* Danh mục */}
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
        <Title level={5} className="!mb-3 text-primary-600 font-medium">Danh mục</Title>
        <Tree
      ...
      {/* Thương hiệu */}
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
        <Title level={5} className="!mb-3 text-primary-600 font-medium">Thương hiệu</Title>
              ...
              <List.Item className="py-2 px-0 border-b border-gray-100 last:border-0">
                ...
                >
                  <div className="flex items-center">
                    ...
                    {brand.productCount && (
                      <span className="ml-1 text-gray-400 text-xs">({brand.productCount})</span>
                    )}
                  </div>
                </Checkbox>
              </List.Item>
      ...
      {/* Khoảng giá */}
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
        <Title level={5} className="!mb-3 text-primary-600 font-medium">Khoảng giá</Title>
        <Slider
          range
          min={0}
          max={20000000}
          step={100000}
          value={priceRange}
          onChange={(values) => handlePriceRangeChange(values as [number, number])}
          tipFormatter={(value) => `${value?.toLocaleString('vi-VN')}đ`}
          className="mb-4"
        />
        <div className="flex justify-between text-sm">
          <span className="font-medium">{priceRange[0].toLocaleString('vi-VN')}đ</span>
          <span className="font-medium">{priceRange[1].toLocaleString('vi-VN')}đ</span>
        </div>
      </div>
      ...
      {/* Đánh giá */}
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
        <Title level={5} className="!mb-3 text-primary-600 font-medium">Đánh giá</Title>
        <Radio.Group 
          value={minRating}
      ...
      {/* Tính năng */}
      <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
        <Title level={5} className="!mb-3 text-primary-600 font-medium">Tính năng</Title>
        <Checkbox.Group
