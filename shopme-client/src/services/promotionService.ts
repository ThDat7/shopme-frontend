...
class PromotionService extends BaseService {
  ...
  async getPromotionsList() {
    try {
      const response = await this.get<PromotionListItem[]>(
  ...
  async getAllPromotions() {
    try {
      const response = await this.get<Promotion[]>(
        `${API_ENDPOINTS.PROMOTIONS}/active`
      )
      return response.result
    } catch (error) {
      this.handleError(error)
  ...
  async getHeaderPromotions() {
    try {
      const response = await this.get<Promotion[]>(
        `${API_ENDPOINTS.PROMOTIONS}/header`
      )
      return response.result
    } catch (error) {
      this.handleError(error)
  ...
  async getPromotionDetailByAlias(alias: string) {
    try {
      const response = await this.get<PromotionDetailResponse>(
        `${API_ENDPOINTS.PROMOTIONS}/alias/${alias}`
      )
      return response.result
    } catch (error) {
      this.handleError(error)
  ...
  async getCurrentPromotionTypesActive() {
    try {
      const response = await this.get<PromotionIdWithTypeResponse[]>(
        `${API_ENDPOINTS.PROMOTIONS}/types`
      )
      return response.result
    } catch (error) {
      this.handleError(error)
  ...
  async getPromotionProducts(id: number, params: Record<string, any> = {}) {
    try {
      ...
      const url = `${API_ENDPOINTS.PROMOTIONS}/${id}/products${
